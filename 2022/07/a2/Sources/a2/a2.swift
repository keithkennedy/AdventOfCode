...
public struct a2 {
    ...
    public static func main() throws -> Void {
        let terminalOutput = try NSString(contentsOfFile: "input_demo.txt", 
...
func change(directory: String) {
    if directory == ".." {
        if let parent = currentDirectory.parent.first {
        ...
        let newDir = Directory(name: directory, parent: currentDirectory)
